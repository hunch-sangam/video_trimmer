...
class _ScrollableTrimViewerState extends State<ScrollableTrimViewer>
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        ...
        videoActualLength = totalDuration.inMilliseconds;
        ...
        log('Video Selected Duration: $_videoEndPos \n $_videoStartPos');
  ...
  void _onDragEnd(DragEndDetails details) {
    log('onDragEnd');
    if(_videoEndPos.round() > videoActualLength){
      _videoEndPos = videoPlayerController.value.duration.inMilliseconds.toDouble();
      _videoStartPos = (_videoEndPos - const Duration(seconds: 90).inMilliseconds).toDouble();
    }
    _scrollStartTimer?.cancel();
    _scrollingTimer?.cancel();
    ...
    setState(() {
      ...
      if (_dragType == EditorDragType.right) {
        log('Seek video to end position ${Duration(milliseconds: _videoEndPos.toInt())}');
        videoPlayerController
            .seekTo(Duration(milliseconds: _videoEndPos.toInt()));
      } else {
        log('Seek video to start position ${Duration(milliseconds: _videoStartPos.toInt())}');
        videoPlayerController
  ...
  void dispose() {
    ...
    _scrubberAnimation?.removeStatusListener((e){});
